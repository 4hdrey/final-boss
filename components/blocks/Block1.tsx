...
export default function Block1(
  ...
  return (
    <div className="h-52 flex flex-row border-none text-text my-2">
      <textarea
        className="h-full overflow-scroll p-4 block resize-none rounded-r-xl rounded-l-md bg-main focus:outline-none focus:shadow-md "
        style={blocks[blockId][2][0]}
        ref={leftElem}
        value={blocks[blockId][1][0]}
        onChange={(e) => handleTextEdit(e)}></textarea>
      <div
        className="w-1 hover:cursor-col-resize relative flex"
        ref={resizeRef}>
        <button
          className="w-2 h-2 absolute top-0 right-3 m-1"
