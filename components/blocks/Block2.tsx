...
export default function Block2(props: blockProps<[number, string[], React.CSSProperties[]][]>) {
  ...
  return (
        <div className="w-full h-52 flex flex-row my-2 text-text">
          <div className="flex flex-row w-96 h-52"
            ...
            <textarea className="h-full overflow-scroll p-4 block resize-none bg-main rounded-l-md focus:outline-none focus:bg-focus"  
            ...
            <textarea className="h-full w-full flex-1 overflow-scroll p-4 block resize-none bg-main rounded-r-xl focus:outline-none focus:bg-focus" 
                      value={blocks[blockId][1][1]}
                      onChange={(e) => handleTextEdit(e, 2)}
            ></textarea>
          </div>
          <div className="w-1 hover:cursor-col-resize relative" ref={resizeRef2}>
            <button className="w-2 h-2 bg-red-600 absolute top-0 right-3 m-1"
