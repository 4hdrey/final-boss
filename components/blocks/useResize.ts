...
export default function useResize(
  refLeft: React.RefObject<Element> | React.RefObject<HTMLDivElement>,
  refResize: React.RefObject<HTMLDivElement>,
  blocks: [number, string[], React.CSSProperties[]][],
  blockId: number,
  onBlockModify: React.Dispatch<
    React.SetStateAction<[number, string[], React.CSSProperties[]][]>
  >,
  size: number
): void {
  const [leftWidth, setLeftWidth] = useState<number>(0);
  const [newLeftWidth, setNewLeftWidth] = useState(
    blocks[blockId][2][size].width
  );
  const [style, setStyle] = useState<React.CSSProperties>(
    blocks[blockId][2][size]
  );
  const [x, setX] = useState(0);

  useEffect(() => {
    var newSize = blocks.map<[number, string[], React.CSSProperties[]]>(
      (block, index) => {
        ...
        if (index === blockId) {
          switch (blockSize) {
            ...
            case 3:
              if (size === 0)
                return [block[0], block[1], [style, block[2][1], block[2][2]]];
              else if (size === 1)
                return [block[0], block[1], [block[2][0], style, block[2][2]]];
  ...
  function handleResize(event: MouseEvent) {
    ...
    var newWidth =
      ((leftWidth + dx) * 100) /
      refLeft.current!.parentElement!.getBoundingClientRect().width;
  ...
  if (refLeft.current !== null && refResize.current !== null)
    refResize.current!.addEventListener("mousedown", handleMouseDown);
}
